...
function addScopeFromNode(
  ...
  while (current) {
    if (current.from <= selection.from && current.to >= selection.to && HYBRID_SCOPE_NODES.has(current.name)) {
...
function getHeadingDecoration(name: string): Decoration | null {
  const level = name.startsWith('ATXHeading') ? name.slice('ATXHeading'.length) : name.slice('SetextHeading'.length)
