...
| `disabled`               | `boolean`                 | —         | 是否禁用用户发起的编辑与图片拖拽/粘贴输入。外部 `modelValue` 更新仍会同步到已挂载实例。                                                                                                                 |
| `readonly`               | `boolean`                 | —         | 是否只读。保留聚焦、选中、复制等非写入交互，但阻止文档修改。                                                                                                                                            |
...
| `focus`             | `()`              | 编辑器获得焦点时触发。                                                 |
| `blur`              | `()`              | 编辑器失去焦点时触发。                                                 |
| `drop`              | `(files: File[])` | 拖拽或粘贴图片文件进入编辑器时触发。当前作为兼容事件统一承载两种来源。 |
...
| `insertText`       | `(text: string) => void`                                     | 在当前光标位置插入文本。如果当前存在选区，选区内容将被替换。插入后光标会自动移动到文本末尾。若编辑器处于 `readonly` 或 `disabled` 状态，该方法不会修改文档。 |
...
| `undoHistory`      | `() => boolean`                                              | 执行一次撤销；若当前没有可撤销历史，或编辑器处于 `readonly` / `disabled` 状态，则返回 `false`。                                                              |
| `redoHistory`      | `() => boolean`                                              | 执行一次重做；若当前没有可重做历史，或编辑器处于 `readonly` / `disabled` 状态，则返回 `false`。                                                              |
| `toggleFullscreen` | `(mode?: 'browser' \| 'screen') => void`                     | 切换全屏模式（支持填满浏览器窗口，或通过原生 API 填满整个屏幕）。                                                                                            |
| `exitFullscreen`   | `() => void`                                                 | 退出全屏状态。                                                                                                                                               |
| `getEditorView`    | `() => EditorView \| undefined`                              | 【高级兼容接口】获取底层的 CodeMirror 6 `EditorView` 实例对象。常规集成应优先使用组件公开的高层方法；直接修改文档可能导致 Vue 的 `v-model` 状态不同步。      |
